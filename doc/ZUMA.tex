...
\begin{xltabular}{\textwidth}{lcR}
    ...
    \multicolumn{2}{l}{\emph{Module Separation}} & \vcenterothercols{Whether the resources of that type should be implemented hierarchically in the additional overlay fabric (\inlinepath{packedOverlay.v}). All combinations of turned on flags are possible.} \\
    \cline{1-2}
    \configvalue{hierarchyNode} & Boolean & One module per ODG node. \\
    \configvalue{hierarchyInterConnect} & Boolean & One module per cluster interconnect block (IIB). \\
    \configvalue{hierarchyBle} & Boolean & One module per BLE. \\
    \configvalue{hierarchyCluster} & Boolean & One module per cluster\,/\,CLB. \\

    \cline{1-2}
    \multicolumn{2}{l}{\emph{Black box replacement}} & \vcenterothercols{Whether the resources of that type should be replaced by black boxes in the additional overlay fabric (\inlinepath{packedOverlayBlackBox.v}) -- useful later reorganization with tools such as RapidWright. Not all combinations are possible, because the highest layer will win, e.g. having the cluster replaced with a blackbox and a ODG node, the cluster will win over the node.} \\
    \cline{1-2}
    ...
    \configvalue{blackBoxInterconnect} & Boolean & Replace the cluster interconnect blocks (IIB) with black boxes. \\
    ...
    \configvalue{extractSetupHold} & Boolean & Whether the setup and hold times should be extracted from the SDF file. \\
    \configvalue{setupTime} & Numberstring & If setup times are not read from the SDF file, they can be manually entered here. Example: ``4e-12''. \\
    \configvalue{holdTime}  & Numberstring & If hold times are not read from the SDF file, they can be manually entered here. Example: ``2.0e-9''. \\
