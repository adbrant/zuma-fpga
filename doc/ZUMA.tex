...
\begin{enumerate}
    ...
    \item Include the generated overlay in an FPGA design (cf.~Section~\ref{sec:include_ZUMA_in_project}).
    \item Run the device vendor's EDA tools.
    \item Extract and copy the standard delay format (SDF) file(s) describing the timing properties of the overlay.
    \begin{itemize}
        \item For Vivado, generate the SDF file that contains the routing delay information by issuing the following command, assuming that your top-level design has the name \emph{Top}:
\begin{verbatim}
>???? Top_complete.v
\end{verbatim}
...
\end{verbatim}
        \item Copy the generated file(s) \inlinepath{Top_complete.sdf} or the pair of \inlinepath{Top_with_buffer.sdf} and \inlinepath{Top_no_buffer.sdf}, if applicable, to your \inlinepath{example/} directory.
        \item Edit the parameters \configvalue{sdfFileName}, and \configvalue{sdfFlipflopFileName} in case of an ISE flow, to these names and set \configvalue{sdf} = True in \inlinepath{zuma_config.py}.
    \end{itemize}
