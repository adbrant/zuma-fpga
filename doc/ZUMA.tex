...
\end{figure}
To get such a structure the script parses the RRG file generated by VPR, which contains the outer structure visualized in Figure~\ref{fig:outer_structure_small} as a graph. %, like the one depicted in Figure~\ref{fig:load_graph}.
% \begin{figure}[htbp]
%     \centering
%     \includegraphics[height=.9\textheight]{Figures/loadedGraphGraphviz.pdf}
%     \caption{A routing resource graph (RRG)}
%     \label{fig:load_graph}
% \end{figure}
% E.g the red OPIN with number 4 on (0,1) in Figure~\ref{fig:outer_structure_small}  is the node 10 in the RRG with connections to the channels nodes 58 and 59 (red arrows). ONE might notice that there are sinks and sources with only one undriven IPIN/OPIN on pads and on clusters as well. E.g. on cluster (0,1): node 11 \& 5, 8 \& 2 and on cluster (1,1) node 29 \& 26. These nodes were optimized away when we build the verilog file.
% \\~\\
% The main idea behind ZUMA is that we now try to save this ODG in a Verilog file which represents our unconfigured virtual FPGA, and parse a routing for every node (which input to which output) from VPR and save it in an configuration file.
%Therefore we load the graph 
We load this RRG in the function \command{load_graph(filename)} in the file \inlinepath{InitFpga.py}.
%When \configvalue{orderedIO} = True is set in the \inlinepath{zuma_config.py} file we add permutation MUXes. Normally the later by the configuration used input and output sink/sources are spread over the whole FPGA and are only connected to one OPIN/IPIN.
%We then alter the graph by connect every source and sink of a pad to every other OPIN/IPIN of every other pad.
%Then we can later use the these source and sink nodes as MUXes to configure which input/output pad we want to use for the incoming/outcoming signals of the FPGA.
...
To speed up the reconfiguration process, ZUMA configures blocks of \(32\) LUTs in parallel. Each such block is internally called a stage.
For each stage ZUMA configures each line of the 32 LUTs step-by-step as shown in Figure~\ref{fig:config_p2}.
Therefore, every bit of the \hwport{config_data} input is used for one LUT. ZUMA thus renames that input to \hwport{wr_data} and assigns the proper bit \hwport{wr_data[i]} as input to each LUT of a stage.
\begin{figure}[htbp]
    \centering
    \includegraphics[width=.8\textwidth]{Figures/configuration_part2.pdf}
...
\begin{samepage}
\begin{lstlisting}[style=Verilog,caption={{Excerpt from config\_controller\_simple.v.}}, label={lst:verilog_config_controller_simple.v}]
